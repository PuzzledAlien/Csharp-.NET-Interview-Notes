...
9. [在ASP.NET中服务端控件Literal控件和Label控件的区别是（）？]()
10. [ASP.NET中关于服务端控件Button的事件表述正确的是（）？（可多选）]()
11. [ASP.NET中编写的DBHelper是用来（）？]()
12. [“StudentService”一般属于三层中（）的命名规范？]()
13. [后缀名为“.ashx”的文件属于（）？]()
14. [我们学习的ASP.NET中页面的后缀名是（）？]()
15. [下列不属于ASP.NET数据控件的是（）？]()
16. [DropDownList下拉列表控件中描述正确的是（）？（可多选）]()
17. [MapPath用于获取网站根目录路径,请问它是（）系统对象的属性？]()
18. [以下（）不属于获取页面参数的方式？]()
19. [在ASP.NET中关于用户控件描述不正确的是（）？]()
